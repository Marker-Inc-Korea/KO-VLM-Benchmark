...
def _generate_wrong_answer(
    image_path1, image_path2, image1_context, image2_context, question, gt_answer, api_key, answer_type
):
    """Core function to generate wrong answers using Gemini."""
    ...
    response = client.models.generate_content(
        ...
        contents=[
            types.Part.from_bytes(data=image_bytes1, mime_type="image/jpeg"),
            types.Part.from_bytes(data=image_bytes2, mime_type="image/jpeg"),
            query,
        ],
    )

    if response.text:
        return response.text.replace("\u0304", "")
    return None
...
def generate_wrong_answer_numerical(
    ...
    return _generate_wrong_answer(
        image_path1, image_path2, image1_context, image2_context, question, gt_answer, api_key, "numerical"
    )
...
def generate_wrong_answer_meaning(
    ...
    return _generate_wrong_answer(
        image_path1, image_path2, image1_context, image2_context, question, gt_answer, api_key, "meaning"
    )
...
def generate_wrong_answer_explain(
    ...
    return _generate_wrong_answer(
        image_path1, image_path2, image1_context, image2_context, question, gt_answer, api_key, "explain"
    )
